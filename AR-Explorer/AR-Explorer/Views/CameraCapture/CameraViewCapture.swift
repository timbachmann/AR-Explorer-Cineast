 ...
 */
struct CameraViewCapture: View {
    @Environment(\.presentationMode) var mode: Binding<PresentationMode>
    @StateObject var model = CameraModelCapture()
...
extension CameraViewCapture {
    ...
    func savePhotoToList() {
        ...
        let imageToSaveLocally = ApiImage(id: model.photo.id , data: model.photo.originalData, thumbnail: model.photo.originalData , lat: model.photo.coordinates.latitude, lng: model.photo.coordinates.longitude, date: formatter.string(from: date), source: UIDevice.current.name, bearing: Int(model.photo.heading.trueHeading), score: 0.0, pitch: model.photo.pitch, yaw: model.photo.yaw, publicImage: 0)
...
struct CameraViewCapture_Previews: PreviewProvider {
